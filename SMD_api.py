...
class SMDapi:
    ...
    def check_password(self, username: str, password: str) -> bool:
        try:
            r = requests.get(REQUEST_URL + '/check_password',
    ...
    def get_coins(self, username: str) -> int:
        try:
            r = requests.get(REQUEST_URL + '/get_coins', {'username': username}, cookies=self.SESSION_COOKIES)
            if 'error' in r.json():
                return -1
            else:
                return r.json()['coins']
    ...
    def add_value(self, username: str, coins: int) -> bool:
        r = requests.get(REQUEST_URL + '/add_value', {'username': username, 'coins': coins},
    ...
    def check_user_exists(self, username: str) -> bool:
        r = requests.get(REQUEST_URL + '/check_user_exists', {'username': username}, cookies=self.SESSION_COOKIES)
        if r.text == 'True':
            return True
        else:
            return False
